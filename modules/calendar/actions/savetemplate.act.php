...
//--------------------------------------------------------------------------------------------------
	...
	//if ('saveTemplate' != $_POST['action']) { $page->do404('Action not supported.'); } 
	if (false == array_key_exists('UID', $_POST)) { $page->do404('UID not POSTed.'); }

	$model = new Calendar_Template($_POST['UID']);
	if (false == $model->loaded) { $page->do404("Could not load template $UID");}

	if (false == $user->authHas('calendar', 'calendar_template', 'edit', $model->UID))
		{ $page->do403('You are not authorized to edit this Template.'); }
	...
	else { $page->do301('calendar/edittemplate/' . $model->alias); }
