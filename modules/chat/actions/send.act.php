...
//--------------------------------------------------------------------------------------------------
	...
	if ('' != $report) { $page->doXmlError("Could not send message (database error)"); }
	...
	if (true == $check) { echo "<sent>" . $model->UID . "</sent>"; }
	else { $page->doXmlError("Could not pass message to chat server."); }
