...
//
//	cookies, used to communicate between open windows in the same session
//
//	cookie:chatwindows		// string of chat windows in format ||fromUID|x|y||fromUID|x|y|...
//	cookie:chatmessages		// string of chat message UIDs in format |msgUID|msgUID|msgUID|...
//	cookie:chatupdate		// timestamp, compare to lastupdate
...
function chatInit() {
	...
	timerCycle();			// set the timer going
}
...
function timerCycle() {
	...
	if (awareNetChat == false) return false;
	//----------------------------------------------------------------------------------------------
	//	check cookie:chatupdate, if this value != lastupdate we need to refresh the windows
	//----------------------------------------------------------------------------------------------
	if (lastUpdate != cookieGetChatUpdate() ) {

		// add any new windows which have been created
		windowAddFromCookie();

		// check window positions against cookie
		windowMoveFromCookie();

		// remove any windows that were destroyed
		windowRemoveClosed();

		// display any new messages
		windowCountMessages();

		// prevent this from happening next cycle
		lastUpdate = cookieGetChatUpdate();
	}

	//----------------------------------------------------------------------------------------------
	//	check server for new messages
	//----------------------------------------------------------------------------------------------
	if (timerStepCount >= timerStep) {
		timerStepCount = 0;
		pumpDiv = document.getElementById('pumpDiv');
		var checkMsgUrl = jsServerPath + "chat/checkmsg/since_" + lastMsg + "/" + Math.floor(Math.random()*10000);
		//alert(checkMsgUrl);
		pumpDiv.innerHTML = "<iframe src='" + checkMsgUrl + "' width='10' height='10' frameborder='0'></iframe>";

	}
	
	//----------------------------------------------------------------------------------------------
	//	rinse, repeat (consider using setInterval() here)
	//----------------------------------------------------------------------------------------------
	setTimeout("timerCycle();", timerInterval);	
}
...
//--------------------------------------------------------------------------------------------------
//	create any new windows from cookie:chatwindows
//--------------------------------------------------------------------------------------------------
//	javascript:window/AddFromCookie()

function windowAddFromCookie() {
	//alert('windowAddFromCookie');
	var windowArray = cookieGetWindowArray();
	//alert("windowAddFromCookie - windowArray has " + windowArray.length + "fields");
	for (wIndex = 0; wIndex < windowArray.length; wIndex++) {	
		var fields = windowArray[wIndex];
		windowCreate(fields[0], fields[1], fields[2]); 
	}
}
...
function windowMoveFromCookie() {
	//alert('windowMoveFromCookie');
	var windowArray = cookieGetWindowArray();
	for (i = 0; i < windowArray.length; i++) {	
		var cols = windowArray[i];
		windowSetPosition(cols[0], cols[1], cols[2]);
	}	
}

//--------------------------------------------------------------------------------------------------
//	remove chat windows that have been closed on another page
//--------------------------------------------------------------------------------------------------

function windowRemoveClosed() {
	// for every chat window we have open on the screen
	for (ix = 0; ix < chatWindows.length; ix++) {
		var cols = chatWindows[ix];

		// not in the cookie? close it
		if ((cols[0].length > 0) && (false == cookieWindowExists(cols[0]))) {
			//alert('closing window ' + cols[0] + '(not in cookie)');
			...
			for (oldIndex = 0; oldIndex < chatWindows.length ; oldIndex++) {
				if (chatWindows[oldIndex][0] == cols[0]) { 
					// leave it out
				} else {
					newList[newIndex] = chatWindows[oldIndex];
					newIndex++;
				}
			}	
			chatWindows = newList;
			...
			//	kill the div
			//--------------------------------------------------------------------------------------
			cwDiv = document.getElementById('cw' + cols[0]);
			msgDiv = document.getElementById('msgDiv');
			cwDiv.innerHTML = '';
			msgDiv.removeChild(cwDiv);
...
function windowCreate(fromUser, xPos, yPos) {
	...
	msgDiv.innerHTML = msgDiv.innerHTML
					 ...
					 + " align='right' " + " onClick=\"windowClose('" + fromUser  + "')\" /><br/>"
