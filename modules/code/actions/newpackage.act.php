...
//--------------------------------------------------------------------------------------------------
	...
	//----------------------------------------------------------------------------------------------
	if (false == $kapenta->user->authHas('code', 'code_package', 'new')) {
	...
	//----------------------------------------------------------------------------------------------
	if ('' == $report) {
		$kapenta->session->msg('Created new Package<br/>', 'ok');
		$kapenta->page->do302('/code/editpackage/' . $model->alias);
	} else {
		$kapenta->session->msg('Could not create new Package:<br/>' . $report);
