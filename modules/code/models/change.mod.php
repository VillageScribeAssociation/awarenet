...
class Code_Change {
	...
	var $package;			//_ ref:Code_Package [string]
	var $message;			//_ commit message [string]
	var $files;				//_ list of affected files [string]
	...
	//.	constructor
	//----------------------------------------------------------------------------------------------
	//opt: UID - UID of a Change object [string]

	function Code_Change($UID = '') {
		global $kapenta;
	...
	//----------------------------------------------------------------------------------------------
	//arg: UID - UID of a Change object [string]
	...
	//----------------------------------------------------------------------------------------------
	//.	load Change object serialized as an associative array
	//----------------------------------------------------------------------------------------------
	...
	function extArray() {
		...
		//	links
		//------------------------------------------------------------------------------------------
		if (true == $kapenta->user->authHas('code', 'code_change', 'view', $ext['UID'])) {
			$ext['viewUrl'] = '%%serverPath%%code/showchange/' . $ext['UID'];
			$ext['viewLink'] = "<a href='" . $ext['viewUrl'] . "'>[ more &gt;gt; ]</a>";
		}

		if (true == $kapenta->user->authHas('code', 'code_change', 'edit', $ext['UID'])) {
			$ext['editUrl'] = '%%serverPath%%code/editchange/' . $ext['UID'];
			$ext['editLink'] = "<a href='" . $ext['editUrl'] . "'>[ edit ]</a>";
		}

		if (true == $kapenta->user->authHas('code', 'code_change', 'delete', $ext['UID'])) {
			$ext['delUrl'] = '%%serverPath%%code/delchange/' . $ext['UID'];
