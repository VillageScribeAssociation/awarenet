<? header('HTTP/1.1 403 Forbidden'); exit('403 - forbidden'); /*
<?xml version="1.0" ?>

<page>
	<template>twocol-rightnav.template.php</template>
	<title>[`|pc][`|pc]websiteName[`|pc][`|pc] - galleries - %%schoolName%%</title>
	<content>
		[[:theme::navtitlebox::label=Image Galleries from %%schoolName%%::width=570:]]
		[[:live::river::mod=gallery::view=summarylist::pv=pageNo::allow=orderBy|schoolUID|pagination|num::orderBy=[`|pc][`|pc]orderBy[`|pc][`|pc]::pagination=no::num=3::schoolUID=%%schoolUID%%:]]
	</content>
	<nav1>
		[[:theme::navtitlebox::label=Create New Gallery::toggle=divNewGalleryForm::hidden=[`|sq]yes[`|sq]:]]
		[`|lt]div id=[`|sq]divNewGalleryForm[`|sq] style=[`|sq]visibility: hidden[`|sc] display: none[`|sc][`|sq][`|gt]
		[[:gallery::newgalleryform:]]
		[`|lt]/div[`|gt]
		[`|lt]br/[`|gt]

		[[:theme::navtitlebox::label=My Galleries:]]
		[[:gallery::navlist::userUID=[`|pc][`|pc]userUID[`|pc][`|pc]:]]
		[`|lt]br/[`|gt]

		[[:theme::navtitlebox::label=By School:]]
		[[:gallery::schoolsnav:]]
		<hr/>
		<br/>

		[[:theme::navtitlebox::label=Unsorted Images:]]
		[[:gallery::randomthumbs::userUID=[`|pc][`|pc]userUID[`|pc][`|pc]::num=30:]]
	</nav1>
	<nav2></nav2>
	<script></script>
	<jsinit></jsinit>
	<banner></banner>
	<head></head>
	<menu1>[[:home::menu:]]</menu1>
	<menu2>[[:gallery::menu:]]</menu2>
	<section></section>
	<subsection></subsection>
	<breadcrumb>[[:theme::breadcrumb::label=Galleries - ::link=/gallery/:]]
[[:theme::breadcrumb::label=all - ::link=/gallery/listall/:]]
[[:theme::breadcrumb::label=[`|pc][`|pc]orderLabel[`|pc][`|pc]::link=/gallery/listall/orderBy[`|us][`|pc][`|pc]orderBy[`|pc][`|pc]:]]</breadcrumb>
</page>

*/ ?>
