...
//--------------------------------------------------------------------------------------------------
	...
	//----------------------------------------------------------------------------------------------
	if (array_key_exists('thumb', $i->transforms) == false) {
		$i->loadImage();
		$thumb = $i->scaleToBox(100, 100);
		$thumbFile = str_replace('.jpg', '_thumb.jpg', $i->data['fileName']);
		$i->data['transforms'] .= 'thumb|' . $thumbFile . "\n";
		$i->expandTransforms();
		$i->save();
		...
		header("ETag: \"" . md5($lmDate) . "\"");
		header('Cache-Control: max-age=3600');
		readfile($installPath . $i->transforms['thumb']);
	}
