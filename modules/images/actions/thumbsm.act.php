...
//--------------------------------------------------------------------------------------------------
	...
	//----------------------------------------------------------------------------------------------
	if (array_key_exists('thumbsm', $i->transforms) == false) {
		$i->loadImage();
		$thumb = $i->scaleToBox(50, 50);
		$thumbFile = str_replace('.jpg', '_thumbsm.jpg', $i->data['fileName']);
		$i->data['transforms'] .= 'thumbsm|' . $thumbFile . "\n";
		$i->expandTransforms();
		$i->save();
		...
		header("ETag: \"" . md5($lmDate) . "\"");
		header('Cache-Control: max-age=3600');
		readfile($installPath . $i->transforms['thumbsm']);
	}
?>
