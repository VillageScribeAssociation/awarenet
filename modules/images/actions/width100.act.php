...
//--------------------------------------------------------------------------------------------------
	...
	//----------------------------------------------------------------------------------------------
	if (array_key_exists('width100', $i->transforms) == false) {
		$i->loadImage();
		$thumb = $i->scaleToWidth(100);
		$thumbFile = str_replace('.jpg', '_width100.jpg', $i->data['fileName']);
		$i->data['transforms'] .= 'width100|' . $thumbFile . "\n";
		$i->expandTransforms();
		$i->save();
		...
		header("ETag: \"" . md5($lmDate) . "\"");
		header('Cache-Control: max-age=3600');
		readfile($installPath . $i->transforms['width100']);
	}
