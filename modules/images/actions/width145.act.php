...
//--------------------------------------------------------------------------------------------------
	...
	//----------------------------------------------------------------------------------------------
	if (array_key_exists('width145', $i->transforms) == false) {
		$i->loadImage();
		$thumb = $i->scaleToWidth(145);
		$thumbFile = str_replace('.jpg', '_width145.jpg', $i->data['fileName']);
		$i->data['transforms'] .= 'width145|' . $thumbFile . "\n";
		$i->expandTransforms();
		$i->save();
		...
		header("ETag: \"" . md5($lmDate) . "\"");
		header('Cache-Control: max-age=3600');
		readfile($installPath . $i->transforms['width145']);
	}
