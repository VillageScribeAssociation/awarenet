...
//--------------------------------------------------------------------------------------------------
	...
	//----------------------------------------------------------------------------------------------
	if (array_key_exists('width200', $i->transforms) == false) {
		$i->loadImage();
		$thumb = $i->scaleToWidth(200);
		$thumbFile = str_replace('.jpg', '_width200.jpg', $i->data['fileName']);
		$i->data['transforms'] .= 'width200|' . $thumbFile . "\n";
		$i->expandTransforms();
		$i->save();
		...
		header("ETag: \"" . md5($lmDate) . "\"");
		header('Cache-Control: max-age=3600');
		readfile($installPath . $i->transforms['width200']);
	}
