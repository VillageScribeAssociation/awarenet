...
//--------------------------------------------------------------------------------------------------
	...
	//----------------------------------------------------------------------------------------------
	if (array_key_exists('width290', $i->transforms) == false) {
		$i->loadImage();
		$thumb = $i->scaleToWidth(290);
		$thumbFile = str_replace('.jpg', '_width290.jpg', $i->data['fileName']);
		$i->data['transforms'] .= 'width290|' . $thumbFile . "\n";
		$i->expandTransforms();
		$i->save();
		...
		header("ETag: \"" . md5($lmDate) . "\"");
		header('Cache-Control: max-age=3600');
		readfile($installPath . $i->transforms['width290']);
	}
