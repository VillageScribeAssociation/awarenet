...
//--------------------------------------------------------------------------------------------------
	...
	//----------------------------------------------------------------------------------------------
	if (array_key_exists('width300', $i->transforms) == false) {
		$i->loadImage();
		$thumb = $i->scaleToWidth(300);
		$thumbFile = str_replace('.jpg', '_width300.jpg', $i->data['fileName']);
		$i->data['transforms'] .= 'width300|' . $thumbFile . "\n";
		$i->expandTransforms();
		$i->save();
		...
		header("ETag: \"" . md5($lmDate) . "\"");
		header('Cache-Control: max-age=3600');
		readfile($installPath . $i->transforms['width300']);
	}
