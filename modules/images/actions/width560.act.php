...
//--------------------------------------------------------------------------------------------------
	...
	//----------------------------------------------------------------------------------------------
	if (array_key_exists('width560', $i->transforms) == false) {
		$i->loadImage();
		$thumb = $i->scaleToWidth(560);
		$thumbFile = str_replace('.jpg', '_width560.jpg', $i->data['fileName']);
		$i->data['transforms'] .= 'width560|' . $thumbFile . "\n";
		$i->expandTransforms();
		$i->save();
		...
		header("ETag: \"" . md5($lmDate) . "\"");
		header('Cache-Control: max-age=3600');
		readfile($installPath . $i->transforms['width560']);
	}
