...
class Images_ImageSet {

	//----------------------------------------------------------------------------------------------
	//	memebr variables
	//----------------------------------------------------------------------------------------------

	var $images;			//_	range of serialized Images_Image objects [array]
	...
	function Images_ImageSet($refModule, $refModel, $refUID) {
	...
	//----------------------------------------------------------------------------------------------
	//.	get the index of an image in $this->images give its UID
	//----------------------------------------------------------------------------------------------
	//returns: index on success, -1 on failure [int]

	function getIndex($UID) {
		if (false == $this->loaded) { return (-1); }
		foreach($this->images as $idx => $row) { if ($row['UID'] == $UID) { return $idx; } }
		return (-1);		
	}
	...
	function incWeight($UID) {
		...
		//------------------------------------------------------------------------------------------
		$idx++;
		if (true == array_key_exists($idx, $this->images)) {
			$model->load($this->images[$idx]['UID']);
