...
//--------------------------------------------------------------------------------------------------
	...
	if ('saveCategory' != $_POST['action']) { $page->do404('Action not supported.'); } 
	if (false == array_key_exists('UID', $_POST)) { $page->do404('UID not POSTed.'); }

	$model = new Newsletter_Category($_POST['UID']);
	if (false == $model->loaded) { $page->do404("could not load Category $UID");}

	if (false == $user->authHas('newsletter', 'Newsletter_Category', 'edit', $model->UID))
		{ $page->do403('You are not authorized to edit this Category.'); }
	...
	//----------------------------------------------------------------------------------------------
	if ('' == $report) { $session->msg('Saved changes to Category', 'ok'); }
	else { $session->msg('Could not save Category:<br/>' . $report, 'bad'); }
