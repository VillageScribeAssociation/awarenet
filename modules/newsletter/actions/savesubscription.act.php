...
//--------------------------------------------------------------------------------------------------
//*	save changes to a Subscription object
//--------------------------------------------------------------------------------------------------
	...
	if ('saveSubscription' != $_POST['action']) { $kapenta->page->do404('Action not supported.'); } 
	if (false == array_key_exists('UID', $_POST)) { $kapenta->page->do404('UID not POSTed.'); }

	$model = new Newsletter_Subscription($_POST['UID']);
	if (false == $model->loaded) { $kapenta->page->do404("could not load Subscription $UID");}

	if (false == $kapenta->user->authHas('newsletter', 'Newsletter_Subscription', 'edit', $model->UID))
		{ $kapenta->page->do403('You are not authorized to edit this Subscription.'); }
	...
	foreach($_POST as $field => $value) {
		switch(strtolower($field)) {
			...
			case 'email':	$model->email = $utils->cleanString($value); break;
	...
	//----------------------------------------------------------------------------------------------
	if ('' == $report) { $kapenta->session->msg('Saved changes to Subscription', 'ok'); }
	else { $kapenta->session->msg('Could not save Subscription:<br/>' . $report, 'bad'); }
