...
//--------------------------------------------------------------------------------------------------
	...
	if ('saveSubscription' != $_POST['action']) { $page->do404('Action not supported.'); } 
	if (false == array_key_exists('UID', $_POST)) { $page->do404('UID not POSTed.'); }

	$model = new Newsletter_Subscription($_POST['UID']);
	if (false == $model->loaded) { $page->do404("could not load Subscription $UID");}

	if (false == $user->authHas('newsletter', 'Newsletter_Subscription', 'edit', $model->UID))
		{ $page->do403('You are not authorized to edit this Subscription.'); }
