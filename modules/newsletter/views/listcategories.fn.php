...
function newsletter_listcategories($args) {
	...
	//----------------------------------------------------------------------------------------------
	if (false == $kapenta->user->authHas('newsletter', 'newsletter_category', 'list')) { return ''; }
	...
	$total = $kapenta->db->countRange('newsletter_category', $conditions);	//% total number of items [int]
	$totalPages = ceil($total / $pageSize);					//% number of pages [int]
	$range = $kapenta->db->loadRange('newsletter_category', '*', $conditions, $orderBy, $pageSize, $start);
