...
//postarg: peer - UID of a P2P_Peer object [string]
	...
	if (false == array_key_exists('path', $detail)) { $page->doXmlError('Missing: path'); }
	if (false == array_key_exists('hash', $detail)) { $page->doXmlError('Missing: hash'); }
	if (false == array_key_exists('size', $detail)) { $page->doXmlError('Missing: size'); }
	if (false == array_key_exists('index', $detail)) { $page->doXmlError('Missing: index'); }

	//----------------------------------------------------------------------------------------------
	//	get and return the file part
	//----------------------------------------------------------------------------------------------
	if (false == $kapenta->fileExists($detail['path'])) { $page->doXmlError('File not found.'); }

	$meta = new KLargeFile($detail['path']);
	$raw = $meta->getPart($detail['index']);
	if ('' == $raw) { $page->doXmlError('Part could not be loaded: ' . $detail['index'] . '.'); }
	if (sha1($raw) != $detail['hash']) { $page->doXmlError('Hash mismatch.'); }
