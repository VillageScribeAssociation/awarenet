...
class Polls_Question {
	...
	function loadAnswers() {
		...
		$conditions = array();
		$conditions[] = "question='" . $db->addMarkup($this->UID) . "'";

		$range = $db->loadRange('polls_answer', '*', $conditions, 'weight ASC');
	...
	function hasVoted($userUID) {
		...
		$range = $db->loadRange('polls_vote', '*', $conditions);
	...
	function loadResults() {
		...
		$sql = "select answer, count(UID) as score from polls_vote "
			 . "where `question`='" . $db->addMarkup($this->UID) . "'"
			 . "group by `answer`";
