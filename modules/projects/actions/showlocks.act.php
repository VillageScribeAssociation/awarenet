...
//reqopt: clear - UID of a section to unlock [string]
	...
	if (true == array_key_exists('clear', $kapenta->request->args)) {
		...
		if (false == $model->loaded) {
			$kapenta->session->msg('Unkown project section, could not clear lock.', 'bad');
		} else {
			$model->lockedBy = '';
			$model->lockedOn = '';
			$report = $model->save();
			if ('' == $report) {
				$ext = $model->extArray();
				$kapenta->session->msg('Cleared lock on section: ' . $ext['titleLink'], 'bad');
				$kapenta->page->do302('projects/showlocks/');
			} else {
				$kapenta->session->msg('Could not clear lock:<br/>' . $report, 'bad');
			}
		}
	}
