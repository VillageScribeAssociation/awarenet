...
function revisions__cb_object_deleted($args) {
	...
	if (false == array_key_exists('data', $args)) {
		$kapenta->session->msgAdmin('Record deletion: Missing object data argument.');
		return false;
	}

	if (false == array_key_exists('dbSchema', $args)) {
		$kapenta->session->msgAdmin('Record deletion: Missing dbSchema argument.');
		return false;
	}
	...
	//$kapenta->session->msgAdmin("caught event: object_deleted, arguments check out...");
	...
	if ((false == array_key_exists('archive', $dbSchema)) || ('no' != $dbSchema['archive'])) {
		...
		//$kapenta->session->msgAdmin("recording deletion of " . $dbSchema['model'] . '::' . $data['UID']);
	}
