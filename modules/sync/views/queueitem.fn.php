...
function sync_queueitem($args) {
	...
	switch ($model->type) {
		...
		case "dbUpdate":
				...
				$doc = new KXmlDocument($model->ndata);
				$kids = $doc->getChildren(1);		//% handles to children of root node [array]
				foreach($kids as $childId) {
					$child = $doc->getEntity($childId);
					if ('table' == $child['type']) { 
						$cond .= "<b>Model:</b> " . $child['value'] . "<br/><br/>\n";
					}
					...
					if ('fields' == $child['type']) {
						...
						foreach($fields as $fName => $fVal) 
							{ $fTable[] = array($fName, htmlentities(base64_decode($fVal))); }
	...
	$ext['dataHtml'] = htmlentities($model->ndata);
	$ext['dataHtml'] = str_replace("\n", "<br/>\n", $ext['dataHtml']);
	$ext['dataHtml'] = str_replace("\t", "&nbsp;&nbsp;&nbsp;&nbsp;", $ext['dataHtml']);
