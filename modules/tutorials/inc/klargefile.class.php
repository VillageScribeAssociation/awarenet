...
class KLargeFileTutorials {
	...
	function KLargeFileTutorials($path = '') {
		$this->parts = array();
		$this->path = $path;
	...
	function makeFromFile() {
		...
		for ($i = 0; $i < $numParts; $i++) {
			$raw = $this->getPart($i);
			$hash = sha1($raw);
			$justName = str_replace('/', '-fs-', $this->path);
			$justName = str_replace('\\', '-bs-', $justName);
			$fileName = 'modules/tutorials/assets/parts/' . $justName . '_' . $hash . '.part.php';
			$this->parts[$i] = array(
				'index' => $i,
				'status' => 'pending',
				'hash' => $hash,
			...
			$this->storePart($i, base64_encode($raw), $hash);
		}
	...
	function stitchTogether() {
		...
		foreach($this->parts as $part) {
			$part64 = $kapenta->fileGetContents($part['fileName']); //so that we can deploy on old kapenta systems as well!
	...
	function storePart($index, $content64, $hash) {
		...
		$check = $kapenta->fs->checkName($fileName);
				
		$check = $kapenta->fs->put($fileName, $content64);
		if (false == $check) { echo $fileName . ":file could not be saved<br/>"; return false; }
	...
	function makeMetaFileName($path) {
		...
		$path = 'modules/tutorials/assets/parts/' . $path . '.xml.php';
		return $path;
	}
